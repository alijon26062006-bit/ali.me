...
<p align="center">
  ...
  <img src="docs/miniapp.png" alt="Панель как Mini App внутри Telegram" width="250">
</p>
...
Спрашивается это один раз — дальше `/start` просто напоминает, как пользоваться,
а сменить страну и валюту можно кнопкой в `/settings`.

**Всё делается кнопками.** Внизу чата — постоянное меню: «Сегодня · Неделя · Месяц»,
«Панель · Последние», «Лимиты · Настройки». Валюта, страна, лимиты и экспорт меняются
кнопками внутри этих экранов — команды набирать не нужно, но они продолжают работать:
`/today`, `/week`, `/month`, `/last`, `/undo`, `/app`, `/limits`, `/export`, `/settings`.

**Итоги в боте** — сумма, разбивка по категориям и текстовые полоски.

**Панель открывается прямо в Telegram** — кнопка «📊 Панель» и команда `/app` запускают
её как Mini App: без браузера, без ссылок и пароля. Панель подхватывает тему Telegram
(светлую или тёмную), системная кнопка «Назад» закрывает редактор траты, а нажатия
отзываются вибрацией. Ссылка на браузерную версию остаётся второй кнопкой.
...
Кнопки в боте тоже покрашены по смыслу — Bot API принимает у inline-кнопок поле `style`
(`primary` · `success` · `danger`): синяя — выбрать и открыть, зелёная — поставить лимит,
красная — удалить. Роли собраны в `src/buttons.js`; если сервер стиль не примет,
клавиатура автоматически уходит без цвета.

Мобильная вёрстка, светлая и тёмная тема. Цвет кнопки всегда объясняет действие:
зелёная — записать и сохранить, красная — удалить, приглушённая — второстепенное;
роли описаны в `public/styles.css` и выдаются одной функцией в `public/app.js`.
...
```bash
npm test          # 48 тестов: парсер, даты, API, бот
...
**Mini App.** Если `PUBLIC_URL` начинается с `https://`, панель открывается прямо внутри
Telegram, а вход происходит автоматически по подписанным `initData` — ни ссылок, ни пароля.
Один шаг руками: в [@BotFather](https://t.me/BotFather) выполните `/setdomain` и укажите свой домен.
Без https бот присылает одноразовую ссылку в браузер — всё работает, просто менее нарядно.
...
src/
  ...
  buttons.js    цвета кнопок бота по роли действия
