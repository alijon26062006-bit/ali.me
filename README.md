...
| 📷 фото чека | сумма распознаётся автоматически *(если включён OCR)* |
...
```bash
npm test          # 41 тест: парсер, даты, API, бот
...
| `ANTHROPIC_API_KEY` | нет | — | включает распознавание чеков по фото |
